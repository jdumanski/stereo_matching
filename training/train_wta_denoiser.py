"""
Train a DnCNN disparity denoiser on (WTA_disparity, GT_disparity) pairs from vKITTI.

The denoiser learns to map WTA census cost-volume outputs -> clean GT disparity.
This matches the actual input distribution seen during HQS inference (domain-matched training).
Uses L1 loss for sharper, more precise outputs compared to MSE.

Scene20 is held out from training and used as the test set.

Usage:
    python training/train_wta_denoiser.py

Saves best val checkpoint to models/denoiser_wta_l1.pth.
"""
...
WIN_SIZE          = 11                    # aggregation window for census cost
CENSUS_WIN        = 7                     # census transform window
PATCH_SIZE        = 64
PATCHES_PER_IMG   = 8
TRAIN_FRAC        = 0.9
BATCH_SIZE        = 64
...
OUTPUT_PATH       = "models/denoiser_wta_l1.pth"
...
def compute_wta(rgb_left_path, rgb_right_path):
    """Compute WTA disparity from census cost volume (mimics inference input)."""
    ...
    cost = init_cost_census(L_g, R_g, int(D_MAX), WIN_SIZE, census_win=CENSUS_WIN)
    return np.argmin(cost, axis=2).astype(np.float32)


def build_cache(pairs):
    """Precompute all (wta, gt) pairs and store in RAM."""
    print(f"Precomputing WTA for {len(pairs)} images...")
    cache = []
    for i, (rgb_l, rgb_r, depth) in enumerate(pairs):
        if i % 50 == 0:
            print(f"  {i}/{len(pairs)}")
        wta = compute_wta(rgb_l, rgb_r)
        gt, valid = load_gt_disp(depth)
        cache.append((wta, gt, valid))
    return cache


class WTADataset(Dataset):
    ...
    def __getitem__(self, idx):
        wta, gt, valid = self.cache[idx // self.K]
        H, W = gt.shape
        P = self.P

        # Sample a patch where >70% of GT pixels are valid (not sky/far)
        for _ in range(20):
        ...
        wta_patch = (wta[y:y+P, x:x+P] / D_MAX).astype(np.float32)
        gt_patch  = (gt[y:y+P,  x:x+P] / D_MAX).astype(np.float32)

        return (torch.from_numpy(wta_patch).unsqueeze(0),
...
def main():
    ...
    train_ds = WTADataset(train_cache)
    val_ds   = WTADataset(val_cache, patches_per_img=4)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_mae = train_loss * D_MAX
        val_mae   = val_loss   * D_MAX
        print(f"Epoch {epoch:3d}/{EPOCHS} | train MAE {train_mae:.3f}px | val MAE {val_mae:.3f}px")
